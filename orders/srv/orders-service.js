...
class OrdersService extends cds.ApplicationService {
  ...
  init() {
    const { OrderItems: OrderItems } = this.entities;

    this.before("UPDATE", "Orders", async function (req) {
      const { ID, Items } = req.data;
      if (Items)
        for (let { product_ID, quantity } of Items) {
          const { quantity: before } = await SELECT.one.from(OrderItems, (oi) => oi.quantity).where({ parent_ID: ID, product_ID });
    ...
    this.before("DELETE", "Orders", async function (req) {
      const { ID } = req.data;
      const Items = await SELECT.from(OrderItems, (oi) => {
